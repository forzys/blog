...
export async function onRequest(context) {
    ...
    return CounterMain(request, env.CounterSpace)
}
    

async function CounterMain (request, CounterSpace) {
    ...
    MoeCounterSVG.hit =
        (GetQueryString(url, "value") === "hit" || GetQueryString(url, "value") === "JSON")
            ? await GenSVG (url, CounterJSON, GetQueryString(url, "theme"), "hit", CounterSpace)
            : null;

    MoeCounterSVG.visitor =
        (GetQueryString(url, "value") === "visitor" || GetQueryString(url, "value") === "JSON")
            ? await GenSVG (url, CounterJSON, GetQueryString(url, "theme"), "visitor", CounterSpace)
            : null;
...
async function GenSVG (url, CounterJSON, theme, value, CounterSpace){
    if (GetQueryString(url, "type") === "MoeCounter") {
        CounterJSON[value] = CounterJSON[value].toString().padStart(8, "0").split("")

        let MoeCounterRes = await ReadMoeCounterData(theme, CounterSpace);
...
async function ReadMoeCounterData (theme, CounterSpace) {
