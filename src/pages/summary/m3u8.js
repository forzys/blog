        ...
        $vue.addEventListener('load', () => {
            ...
            methods: {
                ...
                this.ajax({
                    url: this.url,
                    success: (m3u8Str1) => { 
                    m3u8Str = m3u8Str1?.replace(/#EXT-X-DISCONTINUITY[\s\S]*?#EXT-X-DISCONTINUITY/g, '').replace(/#EXT-X-DISCONTINUITY[\s\S]*?#EXT-X-ENDLIST/,'');
                    ...
                    // 检测视频 AES 加密
                    if (m3u8Str.indexOf('#EXT-X-KEY') > -1 && m3u8Str.indexOf('.key')) {
